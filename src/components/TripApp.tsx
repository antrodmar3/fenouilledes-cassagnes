...
import { fetchForecasts, formatForecastDate, formatUpdatedAt, loadCachedForecasts, STAY_END_FORECAST_ID, weatherDescription, type DayForecast, type ForecastSnapshot } from "@/src/services/weather";
...
function PracticalScreen({ setState, weather }: { setState: React.Dispatch<React.SetStateAction<UserState>>; weather: WeatherState }) {
  ...
  const weatherEntries = [
    ...days.map((day) => ({ id: day.id, label: `D${day.number}` })),
    { id: STAY_END_FORECAST_ID, label: "15 ago" },
  ];
